...
# 다음은 각자 상황에 맞게 수정
#
local_dir = Variable.get("local_dir")   # 이를 꼭 본인의 airflow 서버에서 디렉토리로 만들어주어야 함. 실제 프로덕션에서는 공간이 충분한 폴더 (volume)로 맞춰준다
s3_key_prefix = 'insutance'             # 본인의 ID에 맞게 수정
schema = 'insutance'                    # 본인이 사용하는 스키마에 맞게 수정
